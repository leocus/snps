...
class SNPSNodeGene(BaseGene):
    """
    This gene encodes a SPNS neuron with a firing and a forgetting rule
    """
    _gene_attributes = [
        FloatAttribute('c1'),
        FloatAttribute('p1'),
    ...
    def distance(self, other, config):
        d = 0
        for attribute in ["c1", "p1", "d1", "c2", "p2", "d2"]:
            d += abs(getattr(self, attribute) - getattr(other, attribute))
        return d * config.compatibility_weight_coefficient
